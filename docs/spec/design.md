...
- 各マイコンは前コネクタ・後コネクタの2方向と通信する。Stormworz のマイコンは入出力チャンネル空間が単一（`input/output.getNumber/Bool(1..32)`）であるため、**32 チャンネル空間を前フレーム／後フレームに分割**して2方向を表現する（§5.4）。
...
- 各車は**前フレーム**（前コネクタへ送る／前コネクタから受ける情報の集合）と**後フレーム**を持つ。チャンネル空間を前半/後半に分割して両フレームを単一コンポジット空間へ収める（§5.4）。
- 受信フレームの内容は、**送信元（隣接車）が自分のどちらのコネクタから送ったか**を符号で示す（§5.2）。これと「自車のどちら側で受けたか」を併せ、整列/反転と進行方向を導く。
- 物理配線（コネクタ↔コンポジットの結線、前後の対応）はゲーム内ビルド側の責務であり、本プロトコルは**論理的なチャンネル意味**を定義する。最終チャンネル番号は coordinator が確定（§5.4）。

### 5.2 連結マーカー（接続検出＋前後識別）

各車は毎ティック、前後コネクタへ**符号付きマーカー値**を出力する（REQ-COMM-01）。

- **前コネクタへ出力する `link_marker = +1`**、**後コネクタへ出力する `link_marker = -1`**（大きさ 1＝接続生存信号、符号＝送信元コネクタ識別）。
- 受信側の判定（REQ-ORI-01,03）：
...
| `force_release` | bool | マスター強制解放イベント | 取得/新規連結の車 | REQ-MAS-04,05 |
...
前フレーム＝**Number 1〜8 / Bool 1〜8**、後フレーム＝**Number 9〜16 / Bool 9〜16** を推奨。残りは予備。

| 役割 | 前フレーム | 後フレーム | 型 |
|---|---|---|---|
| link_marker | N1 | N9 | Number |
| auth_throttle | N2 | N10 | Number |
| auth_brake | N3 | N11 | Number |
| （予備：速度等） | N4 | N12 | Number |
| master_present | B1 | B9 | Bool |
| sender_gff | B2 | B10 | Bool |
| emergency_brake | B3 | B11 | Bool |
| door_g_right | B4 | B12 | Bool |
| door_g_left | B5 | B13 | Bool |
| room_light | B6 | B14 | Bool |
| spot_on | B7 | B15 | Bool |
| force_release | B8 | B16 | Bool |
...
-- 整列: 受けた側と送った側が逆(前後)なら整列
front_aligned = front_connected and (rx_front_marker < 0)   -- 隣が後(-1)から送信→整列
back_aligned  = back_connected  and (rx_back_marker  > 0)   -- 隣が前(+1)から送信→整列
-- 出力
output.setNumber(CH_LINK_FRONT, 1)   -- 前へ +1
output.setNumber(CH_LINK_BACK, -1)   -- 後へ -1
```
...
### 6.4 マスター調停（M4 / REQ-MAS-02〜10, REQ-EDGE-01,02）

```lua
-- (a) リクエスト・エッジ
if is_cab then
  local rose = (tgl_req and not prev_tgl_req)   -- OFF→ON でトグルが ON 化したエッジ
  local fell = (not tgl_req and prev_tgl_req)
  if rose then                       -- 取得 (REQ-MAS-02)
    is_master = true
    emit_force_release()             -- 他マスターを解放 (REQ-MAS-04)
    acquire_window = WIN             -- 取得直後の外来解放を無視 (自分は降りない)
  elseif fell then                   -- 解放 (REQ-MAS-03)
    is_master = false
  end
end
-- (b) 運転台付き車の新規連結 → 既存マスター解放 (REQ-MAS-05, REQ-EDGE-01)
local new_conn = (front_connected and not prev_fconn) or (back_connected and not prev_bconn)
if is_cab and new_conn then emit_force_release() end
-- (c) force_release 受信処理
local rx_fr = rx_front.force_release or rx_back.force_release
if rx_fr and is_master and acquire_window == 0 then
  is_master = false                  -- 横取りされ降格
  tgl_req   = false                  -- 内部トグルを OFF へ (REQ-MAS-10) → 再取得は OFF→ON 必須(REQ-MAS-09)
end
-- (d) force_release を反対側へ強制中継 (TTL でホップ上限)
if rx_fr and fr_ttl == 0 then fr_ttl = MAX_HOP end
if emitted_this_tick then fr_ttl = MAX_HOP end
local fr_out = (fr_ttl > 0); if fr_ttl > 0 then fr_ttl = fr_ttl - 1 end
-- (e) 集約
acquire_window = math.max(0, acquire_window - 1)
master_present = is_master or rx_front.master_present or rx_back.master_present
```

- **相互排他（REQ-MAS-04,09 / E2,E11）**：取得した車のみ `force_release` を発信し、取得直後 `acquire_window` 中は外来解放を無視する。よって**最後に取得した車が勝ち**、他マスターは降格して `tgl_req=false` に戻るため、ボタン再押下（OFF→ON）まで再取得しない。編成内マスター数は 0/1 に収束。
- **連結過渡（REQ-MAS-05 / E1,E9）**：運転台付き車の新規連結で既存マスターを解放し、いったん 0 マスター（走行不可・安全側）へ落として荒ぶりを防ぐ。運転手の再リクエストで 1 マスターへ。
...
| ID | 事項 | 内容 | 緩和 |
|---|---|---|---|
| R1 | 物理配線とチャンネル対応 | 単一チャンネル空間で前後フレームを2コネクタへ正しく結線できるか（§5.4）。配線方式でフレーム⇔チャンネル反転の可能性 | チャンネル定数を1箇所集約。coordinator が確定しゲーム内検証 |
