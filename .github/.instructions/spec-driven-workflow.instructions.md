---
description: "仕様駆動ワークフローは、要件が綿密に計画され、実装が徹底的に文書化及び検証されることを保証する、構造化されたソフトウェア開発アプローチを提供します。"
applyTo: "**"
---
...
- **`docs/spec/ui_design.md`**: ユーザーインターフェース設計
- **`docs/spec/screen_transitions.md`**: 画面遷移図
...
**言語ルール**
すべてのドキュメンテーション成果物(`ui_design.md`,`requirements.md`,`screen_transitions.md`)およびテンプレートは日本語で生成する必要があります。
...
- [ ] エッジケースと障害をカタログ化する。
  - 包括的なエッジケースマトリクスと潜在的な障害ポイントを文書化する。
- [ ] 信頼度を評価する。
...
- [ ] 技術設計を成果物へ文書化する:
  - **`docs/spec/ui_design.md`**: UIコンポーネント、レイアウト、操作ルール。
  - **`docs/spec/screen_transitions.md`**: 画面間遷移と分岐条件。
...
- [ ] **実装計画を`docs/spec/requirements.md`に追記する:**
...
- [ ] プルリクエストを準備する（該当する場合）:
  ...
  3. 検証成果物と決定記録へのリンク。
  4. 最終的な`ui_design.md`、`requirements.md`、`screen_transitions.md`へのリンク。
- [ ] ワークスペースを最終化する。
...
3. **再計画**:
   - 新しい調査結果に対処するために`requirements.md`内の実装計画を調整する。
4. **実行の再試行**:
